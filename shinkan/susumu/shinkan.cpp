    ...
    // ************************************************** //
        ...
        while (true)
        {
            try
            {
                ...
                // while(kakudo_x > 0.0)
                // {
                //     auto read_data = bno055.read();
                //     auto mag = std::get<2>(read_data);
                //     kakudo_x = double(mag.x());
                // }
                // motor.run(1.0, 1.0);
                // sleep(1_s);
                // motor.left(1.0);
                ...
                // auto bme_data = bme280.read();
                // Altitude<Unit::m> altitude(std::get<0>(bme_data), std::get<2>(bme_data));
                // speaker.write()
